...
| freya-visualizer | homeassistant-freya-visualizer | Up | 0.0.0.0:8765->8765/tcp |
...
| Freya Visualizer Bridge | 8765 | 8765 | TCP (WebSocket) |
...
## 14. VoiceBox Dashboard

The VoiceBox Dashboard is a custom industrial-themed HTML dashboard served from HA's `www` directory and displayed in the HA sidebar as a dedicated panel.

### Access

| Method | URL |
| --- | --- |
| Direct browser | `http://localhost:8123/local/freya-dashboard.html` |
| HA Sidebar | Settings → Dashboards → VoiceBox (Webpage type) |

### Dashboard File

| Path | Description |
| --- | --- |
| `C:\AI_Projects\homeassistant\config\www\freya-dashboard.html` | Main dashboard HTML file |

### Dashboard Pages

| Tab | Contents |
| --- | --- |
| **Overview** | Area cards, weather (St. James MO via Open-Meteo), system status |
| **Living Room** | Awesome Table motorized desk controls |
| **Bedroom** | Bedroom lamp toggle, Voice PE status |
| **Front Door** | Reolink camera, porch light, motion sensors |
| **Energy** | Senmeter CT energy monitoring |
| **Freya** | Full-width real-time FFT voice spectrum analyzer |

### Design

Dark industrial theme with amber/orange accents. Fonts: Bebas Neue, Rajdhani, Share Tech Mono. The Freya tab features a phosphor green spectrum analyzer that displays Freya's real voice FFT during responses.

---

## 15. Freya Visualizer System

The Freya Visualizer is a real-time FFT audio spectrum analyzer that visualizes Freya's voice on the VoiceBox dashboard. It consists of two components: a Docker bridge container and a Windows host mic capture script.

### Architecture

```
DRELANMIC (USB mic near ESP32 speaker)
    ↓ real audio capture
freya_mic_fft.py (Windows host, Python)
    ↓ FFT data via WebSocket
freya-visualizer container (Docker, port 8765)
    ↓ relay to dashboard clients
freya-dashboard.html (VoiceBox dashboard, Freya tab)
```

### Trigger Logic

The system uses HA state changes to control capture timing:

| HA Event | Action |
| --- | --- |
| `assist_satellite` → `responding` | Start mic capture + FFT stream |
| `media_player` → `idle` | Stop mic capture + send idle signal |

This ensures the visualizer only activates during Freya's responses, not during user speech or idle periods.

### Components

#### freya-visualizer (Docker container)

| Property | Value |
| --- | --- |
| Container name | `freya-visualizer` |
| Build path | `C:\AI_Projects\homeassistant\freya-visualizer\` |
| Port | `8765` (WebSocket) |
| Key file | `freya-visualizer\bridge.py` |
| Role | Relay — forwards FFT data from mic script to dashboard clients. Also watches HA WebSocket for state events and broadcasts speaking_start/speaking_end signals. |

Rebuild command:
```powershell
cd C:\AI_Projects\homeassistant
docker compose up -d --build freya-visualizer
```

#### freya_mic_fft.py (Windows host script)

| Property | Value |
| --- | --- |
| Location | `C:\AI_Projects\homeassistant\freya_mic_fft.py` |
| Mic device | DRELANMIC (USB, Generic USB Audio, 48000Hz 24-bit) |
| Sample rate | 48000 Hz |
| FFT bins | 256 (log-spaced 20Hz–8kHz) |
| Frame size | 2048 samples |
| Output rate | 30 fps |
| Noise gate | 0.002 amplitude threshold |

The script captures audio from the DRELANMIC positioned near the ESP32 speaker, runs real FFT analysis, and sends the spectrum data to the bridge over WebSocket.

#### Auto-start (Windows Task Scheduler)

The mic script is registered as a Windows scheduled task that starts at login:

```powershell
# To check status
Get-ScheduledTask -TaskName "Freya Mic FFT"

# To start manually
Start-ScheduledTask -TaskName "Freya Mic FFT"

# To stop
Stop-ScheduledTask -TaskName "Freya Mic FFT"

# To re-register if needed
$action = New-ScheduledTaskAction -Execute "python.exe" -Argument "C:\AI_Projects\homeassistant\freya_mic_fft.py" -WorkingDirectory "C:\AI_Projects\homeassistant"
$trigger = New-ScheduledTaskTrigger -AtLogOn
$settings = New-ScheduledTaskSettingsSet -RestartCount 3 -RestartInterval (New-TimeSpan -Minutes 1)
Register-ScheduledTask -TaskName "Freya Mic FFT" -Action $action -Trigger $trigger -Settings $settings -RunLevel Highest -Force
```

### Physical Setup

The DRELANMIC USB microphone is positioned within a few inches of the ESP32 Voice PE speaker. This ensures Freya's voice dominates the capture and room noise is negligible without requiring aggressive filtering.

### Troubleshooting

| Symptom | Fix |
| --- | --- |
| Visualizer not animating | Check `freya_mic_fft.py` is running: `Get-ScheduledTask -TaskName "Freya Mic FFT"` |
| Bridge disconnected | Run `docker logs freya-visualizer --tail 20` |
| Mic not found | Check DRELANMIC is plugged in, run `python -c "import sounddevice; print(sounddevice.query_devices())"` |
| Wrong WebSocket URL | Check `freya-dashboard.html` line ~791 for `WS_URL = ws://192.168.68.51:8765` |
| Cache showing old version | Chrome → `Ctrl+Shift+Delete` → clear cached images → `Ctrl+Shift+R` |

---

## 16. Conversation History

Freya's full conversation history is automatically saved by the Home Agent integration.

| File | Path |
| --- | --- |
| Raw history (JSON) | `C:\AI_Projects\homeassistant\config\.storage\home_agent.history` |
| Formatted log script | `C:\AI_Projects\homeassistant\freya_conversation_log.py` |
| Formatted log output | `C:\AI_Projects\homeassistant\freya_conversation_log.txt` |

The raw history file is updated automatically after every conversation. To generate a clean human-readable log of the last 7 days:
