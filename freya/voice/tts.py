...
from freya.core.config import TextToSpeechConfig
...
class TextToSpeech:
    ...
    def _resolve_sample_rate(self, voice_path: Path) -> int:
        ...
        logger.debug(
            "Piper sample rate could not be determined from metadata; defaulting to 22050Hz"
        )
        return 22050
...
def create_tts(config: TextToSpeechConfig):
    ...
    if engine == "piper":
        ...
        raise TextToSpeechError(f"Invalid TTS engine '{engine}'. Options: 'piper' or 'elevenlabs'")
