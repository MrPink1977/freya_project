...
class SpeechAgent(BaseAgent):
    ...
    async def start(self) -> None:
        ...
        logger.info("STT initialized: model=%s, device=%s", 
                   self.config.stt.model, self.config.stt.device)
    ...
    async def _initialize_tts(self, engine: TTSEngine) -> None:
        ...
        if engine == TTSEngine.PIPER:
            ...
            logger.info("TTS initialized: engine=Piper, voice=%s", 
                       self.config.tts.voice_path)
            ...
            logger.info("TTS initialized: engine=ElevenLabs, voice=%s, model=%s",
