    ...
    def get_capabilities(self) -> list[AgentCapability]:
        ...
        return [
            AgentCapability(
                ...
                input_topics=["dialog.request", "dialog.clear_context", "dialog.set_model", "dialog.inject_context"],
    ...
    async def _handle_conversation_request(self, message: Message) -> None:
        ...
        try:
            if stream:
                ...
                response, token_count = await self._generate_streaming(messages, model, message.correlation_id)
            else:
                # Non-streaming (fallback)
                response, token_count = await self._generate_non_streaming(messages, model)
            
            # Check for confusion and escalate if enabled
            if self._enable_escalation and self._is_confused(response) and model == self._models["default"]:
                self.logger.info(f"Model {model} confused, escalating to {self._models['reasoning']}")
            ...
            await self.publish_error(
                message,
                exc,
                details={"model": exc.model, "message": "Model not installed. Run `ollama pull {model}`"}
            )
